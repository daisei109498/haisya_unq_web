...
if(isset($_POST['nouhin'])) {
			...
			if (!$row){
				...
				if ($_POST['kanryo']=='1'){
					...
					while($row0 = $statement->fetch()){
					 ...
					 $result = $link->query($sql);
		   		     //driver
					 $sql = "UPDATE juryo_fr_backup set driver_nm ='".$_POST['driver_nm']."' WHERE tenban2='".$_POST['noucd']."' AND date='".date('Y/m/d', $datetime[0])."'" ;
					 // クエリ実行（データを取得）
					 $result = $link->query($sql);
		   		     //time
					 $sql = "UPDATE juryo_fr_backup set kanryo_time ='".date('Y/m/d H:i:s', $datetime[0])."' WHERE tenban2='".$_POST['noucd']."' AND date='".date('Y/m/d', $datetime[0])."'" ;
					 // クエリ実行（データを取得）
					 $result = $link->query($sql);
				}}
			}
	...
	//ドライバー別届け先別グルーピング
	$sql = 'SELECT haisoubi,driver,driver_nm,trip,line_no,todoke,TodokeCd,kanryo,time FROM haisya_info WHERE nin_no="'.$_GET['conditions'].'" group by haisoubi,driver,driver_nm,trip,line_no,todoke,kanryo,time order by driver,line_no';
	$statement = $link -> query($sql);
	...
	$sql = "SELECT * FROM juryo_fr_backup WHERE date='".$nouhinbi."'";
	$statement = $link -> query($sql);
