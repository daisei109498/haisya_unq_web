...
<body bgcolor="#F9F9F9">
    <script>
		  ...
		  android.sendMail("daiseilog-hc-asset_ibaraki@dlog365.onmicrosoft.com");
	function calltel() {
          window.webkit.messageHandlers.telcallbackHandler.postMessage("0488726851");
          window.webkit.messageHandlers.mailcallbackHandler.postMessage("daiseilog-hc-asset_ibaraki@dlog365.onmicrosoft.com");
	}
		...
		<div class="dtable_tr">
			...
			<?php
				if (substr($row2['driver'],0,1) == "k") {
						$mail="mailto:daiseilog-hc-asset_ibaraki@dlog365.onmicrosoft.com";
					}else{
						$mail="mailto:daiseilog-hc-asset_ibaraki@dlog365.onmicrosoft.com";
					}
			?>
			...
			?>
				...
				<input type="hidden" name="noucd" value="<?php echo htmlspecialchars($row2['TodokeCd'],ENT_QUOTES,'UTF-8'); ?>">
				<input type="hidden" name="driver_nm" value="<?php echo htmlspecialchars($row2['driver_nm'],ENT_QUOTES,'UTF-8'); ?>">
							...
							if ($row3['driver_nm']==$row['driver_nm']){
								if ($row3['trip']==$row['trip']){
									if ($row3['line_no']==$row2['line_no']){
										...
										if ($row5['tenban2']==$row2['TodokeCd']){
										?>
										総納品数:<?php echo htmlspecialchars($row5['nohin_yotei'],ENT_QUOTES,'UTF-8'); ?>
										<?php }}}?>
										<?php 
										}else{}
									}else{}
			...
			<?php }?>
				...
				<?php }}?>
				<input class="simple_square_btn" type="button" name="nouhin" value="受領確認" onclick="location.href='haisya_menu.php?driver=<?php echo htmlspecialchars($_GET['conditions'],ENT_QUOTES,'UTF-8'); ?>&todoke=<?php echo $row2['TodokeCd']?>'" disabled>  <br>
			</div>
			</form>
		</div>
		<?php
